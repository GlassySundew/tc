...
/** 
	for proper work of saves, sqlite .hdll lib of the latest
	version with URI enabled has to be compiled(amalgamation)
**/
class Save {
	...
	public static function initFields() {
		saveDirectory = Settings.SAVEPATH + "save/";
	}
	...
	public function upsertLevelMap( name : String, tmxMap : format.tmx.TmxMap ) : SavedLevel {
			...
			if ( sqlite.request( '
					select * from maindb.entities 
						where name = "player_shallow_feet" 
							and level_id = ${entity.model.level.sqlId}' ).hasNext() )

				sqlite.request( '
					update maindb.entities set 
						blob = "${entity.model.footX}_${entity.model.footY}"
					where 
						name = "player_shallow_feet"
						and level_id = ${entity.model.level.sqlId}
				' )
			else
				...
				sqlite.request( '
					insert into maindb.entities (name, blob, level_id) values(
						"player_shallow_feet",
						"${entity.model.footX}_${entity.model.footY}",
						${entity.model.level.sqlId}
					)
				' );
		}

		if ( entity.model.sqlId != null ) {
			...
			sqlite.request( '
				update maindb.entities set
					...
					level_id = ${entity.model.level.sqlId}
					where id = ${entity.model.sqlId}
				' );
		} else {
			...
			entity.model.sqlId = sqlite.request( 'select max(id) id from maindb.entities' ).next().id;
		}
	}
	...
	public function getSavedLevel( level : SavedLevel ) : format.tmx.TmxMap {
		var s = new Serializer();
	...
	public function getSavedLevelByName( name : String ) : format.tmx.TmxMap {
		try {
	...
	public function bringPlayerToLevel( level : SavedLevel ) {
		return
			sqlite.request(
				'update maindb.entities set level_id = ${level.id} where name = "en.player.Player" returning *'
			);
	}
	...
	public function playerSavedOn( level : ServerLevel ) : SavedEntity {
		return
			sqlite.request(
				'select * from maindb.entities where name = "en.player.Player" and level_id = ${level.sqlId}'
			).next();
	}
	...
	public function getPlayerShallowFeet( player : Player ) {
		return
			sqlite.request(
				'select * from maindb.entities where name = "player_shallow_feet" and level_id = ${player.model.level.sqlId}'
			).next();
	}
