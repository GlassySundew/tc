...
import shader.GpuParticleClamped;
import hxd.Math;
...
enum GpuEmitMode {
	...
	**/
	Disc;
	// От-краёв-к-центру плоский диск
	FlatInversedDisc;
}
...
class GpuPartGroup {
	...
	static function getFields( inst : GpuPartGroup ) {
		...
		for ( f in [
			"parent", "sortMode", "emitMode", "needRebuild", "pshader", "partIndex", "particles", "texture", "colorGradient", "amount", "currentParts",
			"ebounds", "maxTime", "lastMove"
		] ) FIELDS.remove( f );
		for ( f in FIELDS.copy() ) if ( Reflect.isFunction( Reflect.field( inst, f ) ) ) FIELDS.remove( f );
	...
	var pshader = new shader.GpuParticleClamped();
	var partIndex = 0;
	var currentParts = 0;
	...
	inline function set_sortMode( v ) {
		needRebuild = true;
		return sortMode = v;
	}
	...
	inline function set_sizeRand( v ) {
		needRebuild = true;
		return sizeRand = v;
	}

	inline function set_sizeIncr( v ) {
		needRebuild = true;
		return sizeIncr = v;
	}

	inline function set_speed( v ) {
		needRebuild = true;
		return speed = v;
	}

	inline function set_speedIncr( v ) {
		needRebuild = true;
		return speedIncr = v;
	}

	inline function set_gravity( v ) {
		needRebuild = true;
		return gravity = v;
	}

	inline function set_speedRand( v ) {
		needRebuild = true;
		return speedRand = v;
	}

	inline function set_life( v ) {
		needRebuild = true;
		return life = v;
	}

	inline function set_lifeRand( v ) {
		needRebuild = true;
		return lifeRand = v;
	}

	inline function set_nparts( n ) {
		if ( n > nparts ) needRebuild = true;
		return nparts = n;
	}

	inline function set_emitLoop( v ) {
		needRebuild = true;
		return emitLoop = v;
	}

	inline function set_emitMode( v ) {
		needRebuild = true;
		return emitMode = v;
	}

	inline function set_emitStartDist( v ) {
		needRebuild = true;
		return emitStartDist = v;
	}

	inline function set_emitDist( v ) {
		needRebuild = true;
		return emitDist = v;
	}

	inline function set_emitAngle( v ) {
		needRebuild = true;
		return emitAngle = v;
	}

	inline function set_emitSync( v ) {
		needRebuild = true;
		return emitSync = v;
	}

	inline function set_emitDelay( v ) {
		needRebuild = true;
		return emitDelay = v;
	}

	inline function set_emitOnBorder( v ) {
		needRebuild = true;
		return emitOnBorder = v;
	}

	inline function set_rotInit( v ) {
		needRebuild = true;
		return rotInit = v;
	}

	inline function set_rotSpeed( v ) {
		needRebuild = true;
		return rotSpeed = v;
	}

	inline function set_rotSpeedRand( v ) {
		needRebuild = true;
		return rotSpeedRand = v;
	}

	inline function set_isRelative( v ) {
		needRebuild = true;
		return isRelative = v;
	}
	...
	public function save() : Dynamic {
		...
		for ( f in getFields( this ) ) Reflect.setField( o, f, Reflect.field( this, f ) );
		return o;
	}
	...
	public function load( version : Int, o : Dynamic ) {
		for ( f in getFields( this ) ) if ( Reflect.hasField( o, f ) ) Reflect.setField( this, f, Reflect.field( o, f ) );
	...
	public function emitPart( rnd : hxd.Rand, pt : GpuPart, absPos : h3d.Matrix ) {
		...
		var size = g.size * ( 1 + srand() * g.sizeRand ),
			rot = srand() * Math.PI * g.rotInit;
		var vsize = g.sizeIncr,
			vrot = g.rotSpeed * ( 1 + rand() * g.rotSpeedRand ) * ( srand() < 0 ? -1 : 1 );

		var life = g.life * ( 1 + srand() * g.lifeRand ),
			time = -rand() * life * ( 1 - g.emitSync ) - g.emitDelay;
		...
		switch( g.emitMode ) {
			...
			case FlatInversedDisc:
				...
				// offsetting the particle
				// var r = g.emitStartDist + g.emitDist * (emitOnBorder ? 1 : Math.sqrt(rand()));
				p.x = 0;
				p.y = 0;
				p.z = speed * ( 1 + g.speedIncr * pshader.stopAt ) * pshader.stopAt;
			case ParentBounds, VolumeBounds, CameraBounds:
...
class GpuParticles extends h3d.scene.MultiMaterial {
	...
	**/
	...
	override function onRemove() {
		super.onRemove();
		for ( p in primitives ) if ( p != null ) p.dispose();
	}

	override function getBoundsRec( b : h3d.col.Bounds ) @:privateAccess
	{
		if ( flags.has( FIgnoreBounds ) ) return super.getBoundsRec( b );
		for ( g in groups ) if ( g.needRebuild ) {
			var s = getScene();
			if ( s != null ) {
				if ( posChanged ) {
					calcAbsPos();
	...
	public function save() : Dynamic {
		var bounds = null;
		for ( g in groups ) switch( g.emitMode ) {
	...
	public function load( _o : Dynamic, ?resourcePath : String ) {
		...
		for ( g in o.groups ) addGroup().load( o.version, g );
		if ( o.bounds != null ) volumeBounds = h3d.col.Bounds.fromValues( o.bounds[0] - o.bounds[3] * 0.5, o.bounds[1] - o.bounds[4] * 0.5,
			o.bounds[2] - o.bounds[5] * 0.5, o.bounds[3], o.bounds[4], o.bounds[5] );
		hideProps = o.hide;
	}

	public function addGroup( ?g : GpuPartGroup, ?material : h3d.mat.Material, ?index ) @:privateAccess {
		if ( g == null ) g = new GpuPartGroup( this );
		...
		if ( index == null ) index = groups.length;
	...
	function set_seed( s ) @:privateAccess {
		if ( groups != null ) for ( g in groups ) g.needRebuild = true;
		return seed = s;
	}

	function set_volumeBounds( v ) @:privateAccess {
		for ( g in groups ) g.needRebuild = true;
		return volumeBounds = v;
	}

	public function rebuild() @:privateAccess {
		for ( g in groups ) g.needRebuild = true;
	}
	...
	public function getGroup( name : String ) {
		for ( g in groups ) if ( g.name == name ) return g;
		return null;
	}
	...
	override function calcAbsPos() @:privateAccess {
		super.calcAbsPos();
		...
		if ( prevX != absPos.tx || prevY != absPos.ty || prevZ != absPos.tz ) {
			...
			prevY = absPos.ty;
			prevZ = absPos.tz;
			for ( g in groups ) if ( g.emitLoop ) g.lastMove = currentTime;
		}
	}

	function rebuildAll( cam ) @:privateAccess {
		var ebounds = null,
			calcEmit = null,
			partCount = 0,
			partAlloc = this.partAlloc;
		bounds.empty();
		duration = 0.;
		...
		for ( gid in 0...groups.length ) {
			...
			if ( calcEmit != g.emitMode ) {
				...
				switch( g.emitMode ) {
					...
					case Cone, Point, Disc, FlatInversedDisc:
						ebounds = null;
				}
			}
			g.ebounds = ebounds;
			...
			if ( maxLife > duration ) duration = maxLife;
			if ( g.emitLoop ) hasLoop = true;
		...
		if ( primitives.length != groups.length ) primitives.resize( groups.length );

		if ( primitiveBuffers.length != groups.length ) primitiveBuffers.resize( groups.length );

		for ( gid in 0...groups.length ) {
			if ( primitiveBuffers[gid] == null
				|| primitiveBuffers[gid].length > STRIDE * partCount * 4 ) primitiveBuffers[gid] = new hxd.FloatBuffer();
			primitiveBuffers[gid].grow( STRIDE * groups[gid].nparts * 4 );
			primitives[gid] = new h3d.prim.RawPrimitive( {
				vbuf : primitiveBuffers[gid],
				stride : STRIDE,
				quads : true,
				bounds : bounds
			}, true );
		...
		for ( g in groups ) g.needRebuild = false;
		rnd.init( seed );
	}
	...
	function cleanParts( g : GpuPartGroup, pneeded : Int, checkMove = false ) @:privateAccess {
		if ( g.maxTime < 0 ) return;
		var p = g.particles;
		var prev = null;
		var ftime = g.maxTime;
		while( p != null && g.currentParts > pneeded ) {
			...
			if ( st > p.time + ftime && ( !checkMove || -p.time < ftime ) ) {
				...
				p.next = partAlloc;
				partAlloc = p;
				if ( prev == null ) g.particles = n; else
					prev.next = n;
				g.currentParts--;
				p = n;
				continue;
	...
	function syncGroup( g : GpuPartGroup, camera : h3d.Camera, prevTime : Float, visible : Bool ) @:privateAccess {
		// emit
		var needSync = false;
		...
		if ( g.currentParts != pneeded ) {
			if ( g.currentParts < pneeded ) {
				if ( g.lastMove == 0 ) cleanParts( g, pneeded );

				var partAlloc = partAlloc;
				while( g.currentParts < pneeded ) {
					var pt = partAlloc;
					if ( pt == null ) pt = new GpuPart(); else
						partAlloc = pt.next;
					g.emitPart( rnd, pt, absPos );
					if ( g.lastMove != 0 ) pt.time = -prevTime /* no delay */ else
						pt.time -= prevTime;
					pt.index = -1;
					pt.next = g.particles;
					g.particles = pt;
			...
			if ( g.currentParts > pneeded && g.lastMove == 0 ) {
				var ftime = g.maxTime;
				if ( ftime < 0 ) g.maxTime = ftime = currentTime;
	...
	override function emit( ctx : h3d.scene.RenderContext ) @:privateAccess {
		for ( i in 0...materials.length ) {
			var m = materials[i];
			var g = groups[i];
			if ( m != null && g.enable && g.currentParts > 0 ) ctx.emit( m, this, i );
		}
	}
	...
	override function sync( ctx ) @:privateAccess {
		...
		// free memory
		if ( partAlloc != null ) partAlloc = partAlloc.next;

		var prev = currentTime;
		currentTime += ctx.elapsedTime;
		if ( prev < duration && currentTime >= duration ) {
			onEnd();
			if ( !allocated ) return; // was removed
		}

		if ( !ctx.visibleFlag && !alwaysSync ) return;
		...
		if ( camera == null ) camera = new h3d.Camera();
		...
		for ( g in groups ) {
			...
			if ( g.emitMode == CameraBounds ) {
				...
				if ( g.isRelative ) g.pshader.transform.load( absPos ); else
					g.pshader.transform.identity();
				g.pshader.offset.set( 0, 0, 0 );
			}
		}

		if ( duration == 0 && !hasPart ) onEnd();
	}

	function get_count() @:privateAccess{
		var n = 0;
		for ( g in groups ) n += g.currentParts;
		return n;
	}

	override function draw( ctx : h3d.scene.RenderContext ) {
		var primitive = primitives[ctx.drawPass.index];
		if ( primitive == null || primitive.buffer.isDisposed() ) return; // wait next sync()
	...
	override function customSerialize( ctx : hxbit.Serializer ) {
		...
		ctx.addFloat( amount );
		if ( resourcePath == null ) ctx.addDynamic( save() );
	}
	...
	override function customUnserialize( ctx : hxbit.Serializer ) {
		...
		groups = [];
		bounds = new h3d.col.Bounds();
		if ( resourcePath != null ) load( haxe.Json.parse( hxd.res.Loader.currentInstance.load( resourcePath ).toText() ), resourcePath ); else
			load( ctx.getDynamic() );
	}
	#end
}
