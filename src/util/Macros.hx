...
class Macros {
	...
	macro public static function buildServerMessagesSignals() : Array<Field> {
		...
		var serverConstructor = getConstructor( fields );
	...
	macro public static function buildEntityCdbAssign() : Array<Field> {
		var fields = Context.getBuildFields();
		var entClass = EregUtil.decapitalizeWithUnderscore(
			Context.getLocalClass().get().name
		);
		var entityConstructor = getConstructor( fields );
