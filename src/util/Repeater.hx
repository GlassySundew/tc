...
class Repeater {
	...
	public function new( fps : Float ) {
		...
		if ( INDEXES == null )
			if ( haxe.rtti.Meta.getType( util.Repeater ).indexes != null )
				INDEXES = [for ( str in haxe.rtti.Meta.getType( util.Repeater ).indexes ) Std.string( str )];
		reset();
		baseFps = fps;
	}
	...
	static function getMeta() : MetaAccess {
		for ( m in Context.getModule( "util.Repeater" ) )
			switch( m ) {
				case TInst( ct, _ ):
					var c = ct.get();
					if ( c.name == "Repeater" ) {
		...
		Context.error( "Unable to getModule util.Repeater", Context.currentPos() );
		return null;
	}
