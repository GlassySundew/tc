...
private class RpInst {
	...
	public function toString() {
		return Repeater.INDEXES[( k >>> 22 )] + "|" + ( k & 0x3FFFFF ) + ": " + frames + "/" + initial;
	}
}

/**
 * The `Repeater` class allows you to manage various state effects that expire after a set amount of time.
 *
 * **Name of cooldowns**
 *
 * Repeater names are either a string or a string + an integer. This allows you to create cooldowns that share a name
 * but are different.
 *
 * *Examples*:
 * `cd.setMs("punch", 400);`
...
class Repeater {
	...
	public static var inst : Repeater;

	/**
	 * The current list of active cooldowns
	 */
	public var cdList : Array<RpInst>;
	...
	public function new( fps : Float ) {
		...
		if ( INDEXES == null )
			if ( haxe.rtti.Meta.getType( utils.Repeater ).indexes != null )
				INDEXES = [for ( str in haxe.rtti.Meta.getType( utils.Repeater ).indexes ) Std.string( str )];
		reset();
		baseFps = fps;
	}
	 ...
	 */
	public function setMs( k : String, milliSeconds : Float, allowLower = true, repeats : Int, ?onComplete : Void -> Void ) {}
	 ...
	 * @param onComplete A callback invoked once this cooldown completes
	 */
	public function setS( k : String, seconds : Float, allowLower = true, repeats : Int, ?onComplete : Void -> Void ) {}
	 ...
	 * @param onComplete A callback invoked once this cooldown completes
	 */
	public function setF( k : String, frames : Float, allowLower = true, repeats : Int, ?onComplete : Void -> Void ) {}
	...
	static function getMeta() : MetaAccess {
		for ( m in Context.getModule( "utils.Repeater" ) )
			switch( m ) {
				case TInst( ct, _ ):
					var c = ct.get();
					if ( c.name == "Repeater" ) {
		...
		Context.error( "Unable to getModule utils.Repeater", Context.currentPos() );
		return null;
	}
	...
	static function key( k : ExprOf<String> ) : ExprOf<Int> {
		...
		return macro {
			...
			// var v = $subIdx;
			// if( v < 0 || v > 0x3FFFFF )
			// 	trace("Warning: out of bounds cooldown sub-index: \""+utils.Repeater.INDEXES[$eIndex>>>22]+"\" + "+v);
			// #end
	...
	public macro function hasSetS( ethis : Expr, k : ExprOf<String>, seconds : ExprOf<Float> ) return
		macro $ethis._hasSetF( ${key( k )}, $ethis.secToFrames( $seconds ) );

	public macro function hasSetMs( ethis : Expr, k : ExprOf<String>, ms : ExprOf<Float> ) return
		macro $ethis._hasSetF( ${key( k )}, $ethis.msToFrames( $ms ) );
	...
	public macro function setMs( ethis : Expr, k : ExprOf<String>, milliSeconds : ExprOf<Float>, repeats : ExprOf<Int>, extra : Array<Expr> ) {
		...
		return macro $ethis._setF( ${key( k )}, $ethis.msToFrames( $milliSeconds ), $allowLower, $repeats, $onComplete );
	}

	public macro function setS( ethis : Expr, k : ExprOf<String>, seconds : ExprOf<Float>, repeats : ExprOf<Int>, extra : Array<Expr> ) {
		var allowLower : Expr = null, onComplete : Expr = macro null;
		for ( e in extra )
			switch( e ) {
				case { expr : EConst( CIdent( v ) ), pos : _ } if ( allowLower == null && ( v == "true" || v == "false" ) ):
		...
		if ( allowLower == null ) allowLower = macro true;
		return macro $ethis._setF( ${key( k )}, $ethis.secToFrames( $seconds ), $allowLower, $repeats, $onComplete );
	}

	public macro function setF( ethis : Expr, k : ExprOf<String>, frames : ExprOf<Float>, repeats : ExprOf<Int>, extra : Array<Expr> ) {
		var allowLower : Expr = null, onComplete : Expr = macro null;
		for ( e in extra )
			switch( e ) {
				case { expr : EConst( CIdent( v ) ), pos : _ } if ( allowLower == null && ( v == "true" || v == "false" ) ):
		...
		if ( allowLower == null ) allowLower = macro true;
		return macro $ethis._setF( ${key( k )}, $frames, $allowLower, $repeats, $onComplete );
	}
	...
	@:noCompletion
	public inline function _setF( k : Int, frames : Float, allowLower = true, repeats = 0, ?onComplete : Void -> Void ) : Void {
		...
		if ( frames <= 0 ) {
			...
			if ( cur != null ) {
				...
				cdList.push( new RpInst( k, frames, repeats ) );
	...
	function _getCdObject( k : Int ) : Null<RpInst> {
		for ( cd in cdList )
			if ( cd.k == k )
				return cd;
		return null;
	...
	inline function unsetObject( cd : RpInst ) {
		cdList.remove( cd );
		cd.frames = 0;
		cd.cb = null;
