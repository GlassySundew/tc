...
typedef Rules = Map<{
	regions_input : CoordinatedIsland,
	regions_output : CoordinatedIsland
},
	{
		inputs : Map<String, Array<TmxGeneratable>>,
		outputs : Map<String, Array<TmxGeneratable>>,
		mapProps : TmxProperties
	}>;

typedef ExtractedLayer = Array<Array<Null<TmxTile>>>;

typedef CoordinatedTile = {
	tile : Null<TmxTile>,
	x : Int,
	y : Int
};
...
class AutoMap {
	...
	public function applyRulesToMap( map : TmxMap ) {
		...
		var appliedTiles : Array<{
			x : Int,
			y : Int
		}> = [];

		function applyRuleToLayer(
			rule : {
				regions_input : CoordinatedIsland,
				regions_output : CoordinatedIsland
			},
			...
			function createLayerIfNotExistsByName( name : String, type : TmxGeneratable ) {
				...
				function newObjectLayer() {
					return LObjectGroup(
						new TmxObjectGroup( TmxObjectGroupDrawOrder.Topdown, [], 0xffffff, 0, name, 0, 0, 0, 0,
