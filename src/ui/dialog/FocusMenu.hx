...
class FocusMenu extends Process {
	...
	override function onResize() {
		...
		if ( centrized ) {
			contentFlow.minHeight = Std.int( Util.hScaled );
			contentFlow.minWidth = Std.int( Util.wScaled );
			contentFlow.paddingTop = -Std.int( Util.hScaled / 4 );
		}
	}
}
