...
class SAT2D {
	...
	public static function testCircleVsCircle(circleA:Circle, circleB:Circle, ?into:ShapeCollision, flip:Bool = false):ShapeCollision {
		...
		var totalRadius = (circle1.transformedRadius
			- circle1.transformedRadius * (1 - circle1.scaleY) * (Math.abs(Math.sin(angle))))
			+ (circle2.transformedRadius - circle2.transformedRadius * (1 - circle2.scaleY) * (Math.abs(Math.sin(angle))));
		...
		if (distancesq < totalRadius * totalRadius) {
			...
			into.separationX = into.unitVectorX * difference * circle1.scaleY * circle2.scaleY;
