...
class Reader {
	...
	public function read(xml : Xml, ?localPath : String) : TmxMap {
		...
		for (element in map.elements) {
			switch( element.name ) {
				case "tileset":
					tilesets.push(resolveTileset(element, null));
				case "layer":
					layers.push(TmxLayer.LTileLayer(resolveTileLayer(element)));
				case "objectgroup":
					layers.push(TmxLayer.LObjectGroup(resolveObjectGroup(element)));
				case "imagelayer":
					layers.push(TmxLayer.LImageLayer(resolveImageLayer(element)));
				case "group":
					layers.push(TmxLayer.LGroup(resolveGroup(element)));
			}
		}
	...
	public function readObjectTypes(xml : Xml, root : TmxMap = null) : Map<String, TmxObjectTypeTemplate> {
		...
		for (type in f.node.objecttypes.nodes.objecttype) {
			...
			for (prop in type.nodes.property) {
				var ptype : TmxPropertyType = if ( prop.has.type ) {
	...
	**/
	...
	private inline function resolveStaggerIndex(input : String) : TmxStaggerIndex {
		switch( input ) {
			case "even":
				return TmxStaggerIndex.Even;
			case "odd":
				return TmxStaggerIndex.Odd;
			default:
				return TmxStaggerIndex.Unknown(input);
		}
	}
	...
	private inline function resolveStaggerAxis(input : String) : TmxStaggerAxis {
		switch( input ) {
			case "x":
				return TmxStaggerAxis.AxisX;
			case "y":
				return TmxStaggerAxis.AxisY;
			default:
				return TmxStaggerAxis.Unknown(input);
		}
	}
	...
	private inline function resolveOrientation(input : String) : TmxOrientation {
		switch( input ) {
			...
			default:
				return TmxOrientation.Unknown(input);
		}
	}

	private inline function resolveColor(input : String) : Int {
		if ( input.charCodeAt(0) == "#".code ) return Std.parseInt("0x" + input.substr(1)); else
	...
	private inline function resolveRenderOrder(input : String) : TmxRenderOrder {
		switch( input ) {
			...
			default:
				return TmxRenderOrder.Unknown(input);
		}
	}
	...
	private function resolveData(input : Fast, isTileData : Bool = true) : TmxData {
		...
		if ( input.has.encoding ) {
			switch( input.att.encoding ) {
				case "base64":
					encoding = TmxDataEncoding.Base64;
				case "csv":
					encoding = TmxDataEncoding.CSV;
				default:
					throw 'Unknown encoding "${input.att.encoding}"'; // encoding = TmxDataEncoding.Unknown(input.att.encoding);
			}
		}
		...
		if ( input.has.compression ) {
			switch( input.att.compression ) {
				case "gzip":
					compression = TmxDataCompression.GZip;
				case "zlib":
					compression = TmxDataCompression.ZLib;
				default:
					throw 'Unknown compression "${input.att.compression}"';
			}
		}
		...
		switch( encoding ) {
			case TmxDataEncoding.None:
				if ( isTileData ) {
					...
					if ( compression == TmxDataCompression.None ) data = Bytes.ofString(getRawData()); else
						data = uncompressData(new StringInput(getRawData()), compression);
				}
			case TmxDataEncoding.CSV:
				if ( isTileData ) {
					...
					throw "CSV encoding available only for tile data";
	...
	private function resolveTileLayer(input : Fast) : TmxTileLayer {
		...
		var layer : TmxTileLayer = new TmxTileLayer((input.hasNode.data ? resolveData(input.node.data) : null),
			(input.has.id ? Std.parseInt(input.att.id) : 0), (input.has.name ? input.att.name : ""), (input.has.x ? Std.parseFloat(input.att.x) : 0),
			(input.has.y ? Std.parseFloat(input.att.y) : 0), (input.has.offsetx ? Std.parseInt(input.att.offsetx) : 0),
			(input.has.offsety ? Std.parseInt(input.att.offsety) : 0), (input.has.width ? Std.parseInt(input.att.width) : width),
			(input.has.height ? Std.parseInt(input.att.height) : height), (input.has.opacity ? Std.parseFloat(input.att.opacity) : 1),
			(input.has.visible ? input.att.visible == "1" : true), (input.has.tintcolor ? parseColor(input.att.tintcolor) : 0xFFFFFF),
	...
	private inline function resolveDraworder(input : String) : TmxObjectGroupDrawOrder {
		switch( input ) {
			case "index":
				return TmxObjectGroupDrawOrder.Index;
			case "topdown":
				return TmxObjectGroupDrawOrder.Topdown;
			default:
				return TmxObjectGroupDrawOrder.Unknown(input);
		}
	}
	...
	private function resolveObjectGroup(input : Fast) : TmxObjectGroup {
		...
		// Workaround to HaxeFoundation/haxe#6822
		var group : TmxObjectGroup = new TmxObjectGroup((input.has.draworder ? resolveDraworder(input.att.draworder) : TmxObjectGroupDrawOrder.Topdown),
			objects, (input.has.color ? Std.parseInt(input.att.color) : null), (input.has.id ? Std.parseInt(input.att.id) : 0),
			(input.has.name ? input.att.name : ""), (input.has.x ? Std.parseFloat(input.att.x) : 0), (input.has.y ? Std.parseFloat(input.att.y) : 0),
			(input.has.offsetx ? Std.parseInt(input.att.offsetx) : 0), (input.has.offsety ? Std.parseInt(input.att.offsety) : 0),
			(input.has.width ? Std.parseInt(input.att.width) : width), (input.has.height ? Std.parseInt(input.att.height) : height),
			(input.has.opacity ? Std.parseFloat(input.att.opacity) : 1), (input.has.visible ? input.att.visible == "1" : true),
			(input.has.tintcolor ? parseColor(input.att.tintcolor) : 0xFFFFFF), resolveProperties(input));
	...
	private function resolveObject(obj : Fast, applyObjectTypeTemplate : Bool = true) : TmxObject {
		...
		// Type specific data.
		var type : TmxObjectType = if ( obj.hasNode.ellipse ) {
			...
			#else
			var gid = (Int64.parseString(obj.att.gid));
			#end
			if ( gid > @:privateAccess TmxTile.FLIPPED_DIAGONALLY_FLAG ) gid *= -1;
			flippedH = (gid & @:privateAccess TmxTile.FLIPPED_HORIZONTALLY_FLAG) / 2 == @:privateAccess TmxTile.FLIPPED_HORIZONTALLY_FLAG;
			if ( flippedH && gid < 0 ) gid = -gid;
			flippedV = (gid & @:privateAccess TmxTile.FLIPPED_VERTICALLY_FLAG) / 2 == @:privateAccess TmxTile.FLIPPED_VERTICALLY_FLAG;
			if ( flippedV && gid < 0 ) gid = -gid;
			TmxObjectType.OTTile(Int64.toInt(gid & ~(@:privateAccess TmxTile.FLIPPED_HORIZONTALLY_FLAG | @:privateAccess TmxTile.FLIPPED_VERTICALLY_FLAG | @:privateAccess TmxTile.FLIPPED_DIAGONALLY_FLAG)));
	...
	private function resolveImageLayer(input : Fast) : TmxImageLayer {
		var layer : TmxImageLayer = new TmxImageLayer((input.hasNode.image ? resolveImage(input.node.image) : null),
			(input.has.id ? Std.parseInt(input.att.id) : 0), (input.has.name ? input.att.name : ""), (input.has.x ? Std.parseFloat(input.att.x) : 0),
			(input.has.y ? Std.parseFloat(input.att.y) : 0), (input.has.offsetx ? Std.parseInt(input.att.offsetx) : 0),
			(input.has.offsety ? Std.parseInt(input.att.offsety) : 0), (input.has.width ? Std.parseInt(input.att.width) : width),
			(input.has.height ? Std.parseInt(input.att.height) : height), (input.has.opacity ? Std.parseFloat(input.att.opacity) : 1),
